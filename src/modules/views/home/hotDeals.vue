<template>
  <ion-page>
    <ion-header class="hotDeals_page_header">
      <ion-toolbar color="transparent">
        <ion-buttons slot="start">
          <ion-back-button text="" default-href="/main/home"></ion-back-button>
        </ion-buttons>
        <p class="header_title">{{ $t('hot_deals') }}</p>
    ...
    <ion-content :fullscreen="true">
      <ion-grid class="hotDeals_list">
        <ion-row class="product_list">
          <ion-col v-for="item in allHotDeals" v-bind:key="item.id" class="hotDeal_item">
...
import { hotDeals } from '../../../services/shared/graphql';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const { mutate: fetchAllHotDeals } = useMutation(hotDeals);
    ...
    storage.getUser().then((res) => {
      ...
      fetchAllHotDeals({
        ...
        .then((res) => {
          res.data.getHotDiscountProductsB2C.map((item) => {
            const prices = [];
