<template>
  <ion-page>
    <ion-header class="topPicks_page_header">
      <ion-toolbar color="transparent">
        <ion-buttons slot="start">
          <ion-back-button text="" default-href="/main/home"></ion-back-button>
        </ion-buttons>
        <p class="header_title">{{ $t('top_picks') }}</p>
    ...
    <ion-content :fullscreen="true">
      <ion-grid class="topPicks_list">
        <ion-row class="product_list">
          <ion-col class="topPicks_item" v-for="item in allTopPicks" v-bind:key="item.id">
...
import { topPicks } from '../../../services/shared/graphql/typesOfProducts';
...
export default defineComponent({
  ...
  async setup() {
    ...
    const { mutate: fetchAllTopPicks } = useMutation(topPicks);
    let skeleton_flag = ref(false);
    const allTopPicks = ref([]);
    ...
    storage.getUser().then((res) => {
      ...
      fetchAllTopPicks({
        ...
        collection_point: selectedHub.value?.id || undefined,
        state: currentUser.country.id === 127 ? selectedState.state : undefined
      })
        .then((res) => {
          res.data.getPopularProducts.map((item) => {
            const prices = [];
