<template>
  <ion-page>
    <ion-content class="order_collected_container" :scroll-events="true" @ionScroll="getScrollEvent($event)">
      <div v-if="Object.keys(collected_order).length > 0">
        <div class="back-to-top" :class="showBackToTop ? 'show' : ''" @click="clickBackToTop">
          <span>{{ $t('back_to_top') }}</span>
        </div>
        <OrderItem
          :orders="collected_order"
      ...
      <ion-grid v-else class="empty-content">
        ...
        <ion-label> {{ $t('order.have_no_collected') }} </ion-label>
        <ion-label>
...
import {
  ...
  updatePreOrderItemCollectedReadStatus
...
export default defineComponent({
  ...
  async setup() {
    ...
    const updateOrderItemCollectedReadStatus = () => {
      _updatePreOrderItemCollectedReadStatus({ user_id: currentUser.id })
        .then(() => {})
        .catch((error) => {
          console.error(error);
        });
    ...
    const getOverViewData = (pageOffset) => {
      fetchAllOrdersPaginate({
        status_id: [ORDER_STATUS.COLLECTED],
        ...
        .then((res) => {
          ...
          const orders = Object.keys(collected_order.value).length > 0 ? collected_order.value : {};
          response.map((item) => {
            const order = item;
            let currentOrder;
    ...
    if (currentUser.id) {
      ...
      getOverViewData(collected_count);
      collected_count += 5;
    }
    const loadData = (ev) => {
      setTimeout(() => {
  ...
  methods: {
                ...
                this.fetchCancelB2cOrderItem({ id: cancel_id })
                  .then(() => {
                    ...
                    setTimeout(() => {
                      ...
                      this.collected_order = {};
                      this.getOverViewData(this.collected_count);
                      this.collected_count += 5;
                    }, 2500);
                  })
                  .catch((error) => {
    ...
    clickBackToTop() {
      ...
      document.querySelector('ion-content.order_collected_container').scrollToTop(1500);
    },
    gotoCategory() {
